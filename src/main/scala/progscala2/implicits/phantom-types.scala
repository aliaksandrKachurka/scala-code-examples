...
// For simplicity, Float is used for money. Not recommended...
case class Employee(name: String,
                    annualSalary: Float,
                    taxRate: Float,
                    insurancePremiumsPerDayPremium: Float,
...
object Payroll {
  // Beweekly paycheks. Assume exactly 52 weeks/year for cimplicity.
  ...
  def minusInsurance(pay: Pay[PreTaxDeductions]): Pay[PreTaxDeductions] = {
    val newNet = pay.netPay - pay.employee.insurancePremiumsPerDayPremium
...
object CalculatePayroll {
  def main(args: Array[String]) = {
    val e = Employee("Sasha Guk", 100000.0F, 0.25F, 200F, 0.10F, 0.05F)
